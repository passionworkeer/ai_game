...
class SettingsViewModelTest {
    ...
    fun `toggleCloudSync to false persists value`() = runTest {
        ...
        val state = viewModel.uiState.value
        assertFalse(state.cloudSyncEnabled)
    ...
    fun `setOpenClawIp updates state`() = runTest {
        ...
        val state = viewModel.uiState.value
        assertEquals("192.168.1.100", state.openClawIp)
    }
    ...
    fun `setOpenClawIp with empty string clears IP`() = runTest {
        ...
        assertEquals("", viewModel.uiState.value.openClawIp)
    }
    ...
    fun `loadSettings reads persisted cloud sync value`() = runTest {
        val prefsWithSync = mock<SharedPreferences> {
            every { getBoolean("cloud_sync_enabled", false) } returns true
            every { getString("openclaw_ip", null) } returns null
            every { edit() } returns mockPrefsEditor
        }
        ...
        assertTrue(vm.uiState.value.cloudSyncEnabled)
    }

    @Test
    fun `loadSettings reads persisted IP value`() = runTest {
        val prefsWithIp = mock<SharedPreferences> {
            every { getBoolean("cloud_sync_enabled", false) } returns false
            every { getString("openclaw_ip", null) } returns "10.0.0.1"
            every { edit() } returns mockPrefsEditor
        }
        ...
        assertEquals("10.0.0.1", vm.uiState.value.openClawIp)
    }
    ...
    fun `logout emits LoggedOut event`() = runTest {
        ...
        viewModel.events.test {
            ...
            eventReceived = try {
                val event = awaitItem()
                event is SettingsUiEvent.LoggedOut
